"""Assemble the self-contained explorer page: template + model + presets + context + engine + UI.

No network resource is referenced; the page opens from file:// and meets the Artifact page contract.
Every preset path must exist in the engine's state, so a typo cannot silently do nothing.
"""
...
def main():
    ...
    blocks = {"/*MODEL*/": json.dumps(model, separators=(",", ":")), "/*PRESETS*/": json.dumps(presets, separators=(",", ":")),
              "/*CONTEXT*/": json.dumps(context, separators=(",", ":")), "/*ENGINE*/": engine, "/*UI*/": (HERE/"ui.js").read_text()}
    ...
    print(f"explorer.html: {len(page)/1e6:.2f} MB, {len(presets['presets'])} presets, {len(context['items'])} context items")
