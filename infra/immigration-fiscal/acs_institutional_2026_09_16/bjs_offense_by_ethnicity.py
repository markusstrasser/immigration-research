...
pop = {("2009", "white"): 199.9e6, ("2009", "hispanic"): 48.4e6, ("2021", "white"): 191.7e6, ("2021", "hispanic"): 62.6e6}
rows = []
for g in ("white", "hispanic"):
    a, b = counts[("2009", g)], counts[("2021", g)]
    for off in ("total", "violent", "murder", "sexual", "robbery", "assault", "property", "burglary", "mvt", "fraud", "drug", "public_order"):
        r09, r21 = 1e5 * a[off] / pop[("2009", g)], 1e5 * b[off] / pop[("2021", g)]
        rows.append(dict(group=g, offence=off, n2009=a[off], n2021=b[off], count_change_pct=round(100 * (b[off] / a[off] - 1), 1),
