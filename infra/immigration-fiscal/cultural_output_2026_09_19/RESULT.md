...
winners are **6.25% of US-citizen award winner-years since 2015** against a Hispanic
share of the BA+ population of 9.6%, up from 2.91% against 6.2% before
2015: below the credentialled benchmark throughout, rising, and with a post-2015 step this
design cannot separate from a selection-rule change. On variety: **Mexican-cuisine variety per head is nearly flat in a metro's Mexican-origin share (elasticity 0.18, SE 0.03, 27 SE below one), with three quarters of the fitted maximum density reached by a 10% local share and 95% of the lowest-decile metros (mean 0.7% Mexican-origin) already carrying at least one Mexican restaurant. The saturation hypothesis is SUPPORTED at the local margin and NOT IDENTIFIED for the national counterfactual the operator posed**. Mexico-born workers are
...
1.4% of winner-years and otherwise **imputed** from the Census 2010 surname file at
70% Hispanic or more. 20% of winners carry a surname absent from that file and are
...
|---|---|---|---|---|---|---|---|
| all winners | 1990-2014 | 1,009 | 3.47% | 3.79% | 0.89% | 3.57% / 2.38% | 19.0% |
| all winners | 2015-2025 | 457 | 7.22% | 8.00% | 2.41% | 7.44% / 6.13% | 23.4% |
| US citizens only | 1990-2014 | 858 | 2.91% | 3.48% | 0.58% | 2.91% / 2.21% | 16.3% |
| US citizens only | 2015-2025 | 336 | 6.25% | 7.58% | 0.89% | 6.55% / 4.76% | 17.6% |
...
gated by credentials, US-citizen award shares run at 0.47 of the benchmark before
2015 (2.91% against 6.2%) and 0.65 after (6.25% against
9.6%). Against the **adult population** the ratios are 0.22 and
0.36. Representation is below both benchmarks in both periods and rising faster
than either.
...
(6.62% then 9.92%, against 2.91% then 6.25%; counts
10/151 and 12/121) [CALCULATION], so part of any "Hispanic canon presence" comes
...
winner-years is 0.89% post-2015, against a Mexican-origin share of US adults 25-64 of
...
Coverage of this run: 32 states fetched, 271,624 restaurant and fast-food features, 261,400 of them inside a CBSA, 82% carrying a cuisine tag. The estimates below use the 573 CBSAs of 935 whose every state was fetched; a metro whose state was only partly fetched would show a spuriously low restaurant count, and since the fetched states skew towards high Mexican-origin shares that contaminates the all-CBSA fit upward, to 0.384 (SE 0.045); it is reported in the table file but is not the estimate to read. 19 states were not fetched in this run (AK, MO, NC, NE, NH, OH, OK, OR, PA, RI, SC, SD, TN, VA, VT, WA, WI, WV, WY), so their metros are excluded from the primary estimate rather than entered with partial counts. The covered metros hold 88% of the Mexican-origin population living in CBSAs, and are 15.3% Mexican-origin against 4.1% in the excluded metros [CALCULATION]: the excluded set is the lower-share half, so including it would add low-share metros that already carry Mexican restaurants and would if anything flatten the curve further.
...
|---|---|---|---|
| primary (cuisine tag) | 0.181 | 0.030 | -27.5 |
| independent name classifier | 0.142 | 0.029 | -29.7 |
| Mexican share of cuisine-tagged restaurants | 0.208 | 0.015 | -52.2 |

An elasticity of 1 would mean variety scales one-for-one with group size. The estimate is
0.181, 27 SE below one and 6 SE above zero. Doubling a metro's
Mexican-origin share raises Mexican restaurants per head by about 13%; a tenfold
increase raises it by about 52% [CALCULATION].

Restricting to the 121 fully-covered CBSAs above 250,000 residents, where every metro carries a double-digit tagged-restaurant count and none has zero Mexican restaurants, the elasticity is 0.211 (SE 0.036), still 21.9 SE below one, with the name classifier at 0.175 [CALCULATION]. The flatness is not a small-count artefact of micropolitan areas.

**Placebos** load the way they should: all restaurants -0.029, Chinese -0.077,
Italian -0.064, American -0.004 per capita against the same regressor. Only
...
| Mexican-origin share | predicted Mexican restaurants per 100,000 | share of the maximum |
|---|---|---|
| 0.5% | 5.5 | 32% |
| 1.0% | 7.4 | 44% |
| 2.0% | 9.7 | 57% |
...
Three quarters of the fitted maximum density is reached by a 10% local share and 90% by 20%, and the curve is flat above that. **Availability saturates even earlier**: in the lowest decile of
Mexican-origin share (mean 0.7%), 95% of metros already carry at least one
Mexican restaurant, at 6.9 per 100,000 against 15.4 in the top decile
(mean 56%). A 77-fold difference in group share buys a 2.2-fold
difference in per-capita Mexican-restaurant density.

Concretely, comparing the largest metro in the bottom decile of Mexican-origin share with the largest in the top decile: Buffalo-Cheektowaga, NY is 0.6% Mexican-origin and carries 8.8 Mexican restaurants per 100,000 residents on 102 tagged restaurants; Los Angeles-Long Beach-Anaheim, CA is 34% Mexican-origin and carries 15.5 on 2015 [CALCULATION]. A 53-fold difference in group share, and 1.8 times the density.

**Disconfirmation.** The pre-registered rejection conditions were an elasticity near or above 1,
or density still rising at the highest shares. Neither holds: the elasticity is 0.181,
and the quadratic term is negative (-0.051, SE 0.014), meaning the curve bends down rather
than continuing up. Two further checks that could have broken the result did not. An entirely
independent classifier built on restaurant names rather than cuisine tags gives 0.142
(SE 0.029) against the tag-based 0.181. And OSM coverage, measured against
County Business Patterns establishment counts (NAICS 722511 and 722513, 2023), does not vary
with Mexican-origin share: mapped restaurants over CBP establishments load at +0.017
(t 1.42) on log Mexican share, around a median of 0.72 [CALCULATION].
...
with Mexican share (+0.036, t 2.61), which inflates the tag-based Mexican count at the
...
agree: the name classifier ignores cuisine tags entirely (0.142) and the tagged-offset
specification divides by the tagged count, so tagging density cancels (0.208).
...
ACS 2024 PUMS]. A metro at 0.7% share still sits inside a country at 11%: its cooks, its
