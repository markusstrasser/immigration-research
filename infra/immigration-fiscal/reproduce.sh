...
_cmd_build() {
    ...
    case "$target" in
        ...
        all)
            ...
            # urban housing: Zillow rent × Saiz elasticity (supply leg), then + ACS fb-share (demand
            # treatment, no-key summary file). Both skip gracefully if inputs absent.
            uv run --with duckdb python "$ROOT/build/build_msa_rent_elasticity_panel.py"
            uv run --with duckdb python "$ROOT/build/build_msa_fb_rent_panel.py"
