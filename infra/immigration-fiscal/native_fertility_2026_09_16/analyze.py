...
def prep(tag):
    d = pd.read_csv(HERE / f"panel_{tag}.csv", dtype={"cbsa": str})
...
# ---------------------------------------------------------------- CIS reproduction
say("=" * 100)
say("1. CIS (Camarota & Zeigler, Feb 2021) SPEC REPRODUCTION -- ACS 2015-2019 5-yr, native")
say("   births per 1,000 native women 15-50 on immigrant share of metro population.")
...
reg(d23, "native_birth_rate", "fb_share", CTRL, w="native_women_15_50",
    label="+ controls, pop-weighted")
say("  placebo/mechanism: foreign-born women's own birth rate on fb_share")
...
say("  drop rent from controls (rent is a mediator, not a confounder):")
...
# ---------------------------------------------------------------- shift-share IV
say(""); say("=" * 100)
say("4. SHIFT-SHARE (BARTIK) IV -- 2010 origin-country settlement shares x 2010-2023 national")
say("   inflow by origin. Base year is 2010 (earliest ACS1 B05006 at CBSA); the canonical")
...
labels = {k: v["label"] for k, v in meta.items() if k.endswith("E") and k.startswith("B05006")}
leaves = [k for k, L in labels.items()
          if not L.endswith(":") and k != "B05006_001E"]
leaves = [k for k in leaves if k in v10.columns and k in v23.columns]
v10.index = v10.index.to_series().replace(XW)
S10 = v10[leaves]
tot10 = S10.sum(axis=1)
shares = S10.div(tot10.replace(0, np.nan), axis=0)
nat_growth = v23[leaves].sum() - v10[leaves].sum()      # national inflow by origin 2010->2023
bartik = (shares * nat_growth).sum(axis=1)
...
say(f"  leaf origin countries used: {len(leaves)}   n with instrument: {len(iv)}")
...
try:
    ...
    X = sm.add_constant(iv[["d_fb_share"] + DCTRL])
    Z = sm.add_constant(iv[["bartik"] + DCTRL])
    m2 = IV2SLS(iv.d_native_birth_rate, X, Z).fit()
    say(f"  {'2SLS: d_native_birth_rate on d_fb_share':<44} "
        f"b={m2.params['d_fb_share']:+8.4f}  se={m2.bse['d_fb_share']:6.4f}  "
        f"t={m2.tvalues['d_fb_share']:+6.2f}  n={int(m2.nobs):4d}")
    iv_b = m2.params["d_fb_share"]; iv_se = m2.bse["d_fb_share"]
except Exception as e:
    say(f"  2SLS FAILED: {e}"); iv_b = iv_se = float("nan")
...
for nm, b in (("CIS spec (2015-19, top 50)", b_cis), ("own 2023 cross-section", b_own),
