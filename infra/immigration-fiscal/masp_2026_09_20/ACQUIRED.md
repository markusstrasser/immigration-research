...
## Later same-day analysis and field corrections

The inventory-only status above is superseded by the [completed reconstruction](RESULT.md): 758 adult-child interviews from482 original families. `v12–v23` are initial ethnic self-description mentions; `v25` is closest identity only after multiple mentions, not the single-response question as initially described. `v26–v37` concern description to other people. `v51` is race on forms and cannot measure Hispanic nonidentification. These distinctions were checked in the full question text and actual marginals. Schooling is directly measured by `v140`; `v348` measures respondent/spouse gross income bins, and `v338/v340/v341` past-year respondent/spouse receipt of SSI/public welfare/food stamps.

Principal genealogy excludes informant-country codes lacking value labels; the explicitly conditional alternative is separate. Outcome estimates are unweighted historical local descriptions. Source data and hashes above are unchanged; raw/derived remain ignored. Further generation-by-nonidentification splitting stops at insufficient cell sizes.
