...
EXTRACT_PATH = DATA_DIR / "eclsk2011_early_school_extract.tsv.gz"
PANEL_OUT = DATA_DIR / "eclsk2011_early_school_panel.parquet"
GAPS_OUT = DATA_DIR / "eclsk2011_early_school_surface_gaps.tsv"
MODELS_OUT = DATA_DIR / "eclsk2011_early_school_models.tsv"
...
OUTCOMES = {
    "math_k5_scale": ("MTHFLG", "MSCALK5"),
    "reading_k5_scale": ("RDGFLG", "RSCALK5"),
}
...
def build_panel(df: pd.DataFrame) -> pd.DataFrame:
    female = clean_numeric(df["X_CHSEX_R"]).map({2: 1, 1: 0})
    rows: list[pd.DataFrame] = []

    for outcome, (flag_suffix, score_suffix) in OUTCOMES.items():
        for wave in range(1, 9):
            flag_col = f"X{wave}{flag_suffix}"
            score_col = f"X{wave}{score_suffix}"
...
def run_models(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for outcome, sub in panel.groupby("outcome", observed=True):
        model = smf.wls(
            ...
            data=sub,
            weights=sub["weight"],
        ).fit(cov_type="cluster", cov_kwds={"groups": sub["child_id"]})
