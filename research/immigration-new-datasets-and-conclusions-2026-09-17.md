...
**Final September 17 intake:** `default.zip` completes the request with all 98 fields, one extra field, 8,984 unique matching IDs and full requested codebook coverage. Every requested non-ID value agrees with the audited baseline, closing the remaining export-coverage gap above without changing the results. The separate `ICPSR_20862-V6 (1).zip` still has no respondent data: its seven documentation files exactly duplicate files already held. Further NLS downloading is unnecessary for this request; actual LNS respondent data remain missing. [SOURCE: [completion check and source hashes](../infra/immigration-fiscal/new_datasets_2026_09_17/completion_check.json), generated by `check_completion.py`.]
