...
OLS, calibrated sample, 133 CBSA clusters. [SOURCE: `derived/`, RESULT §2] [CALCULATION] Every IV arm is weak or rejected: the paper's gravity instrument has first-stage F of 0.003 to 8.9 here because Mexican-born counts in the sample tracts fell 3% over these windows, so the enclave-diffusion process it models is not running; the Hansen test rejects at p < 0.001 in the column-6 form; the Mexican shift-share is degenerate with one origin group.
