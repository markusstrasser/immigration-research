...
| Mexico school burden/adult | withheld | `v_three_layer_annual` — scenario-household numerator vs full-microsim denominator mismatch |
| Mexico crude annual (fed − school) | unresolved | `v_three_layer_annual` — needs same-universe school numerator |
...
**New falsifier:** If same-universe K-12 local assignment × school-age share still remains below the federal annual proxy for Mexico `<HS` surge cells, local-flow layer may be secondary at **annual** frequency even when the NAS `<HS` cell is negative.
...
| 2026-06-15 | 13–22 | **Three-layer annual:** `school_burden_per_adult` + `net_crude_federal_minus_school` in tensor; `v_three_layer_annual`. Initial Mexico crude ~−$13.5k/adult used the scenario subset denominator. The later $771/+748 correction is also superseded because it paired a scenario-household numerator with the full microsim denominator. Memos: `immigration-school-burden-per-adult-2026-06-15.md`, `immigration-sweep-cycles-13-22-2026-06-15.md`. **Kill:** do not merge with lifetime NAS; do not cite origin school rows until same-universe rebuilt. |
| 2026-06-15 | 23–32 | **NAS Table 8-13 college+ cells** staged (`HS` +$49k, some college +$205k, BA +$514k) → Mexico **synthetic age-25 benchmark +$45,631/adult** (education-mix weighted). Return-migration haircut scenarios in `life.return_migration_haircut_scenarios`. LPR xlsx still missing. Saiz median ε = 2.54 (269 MSAs). Memo: `immigration-sweep-cycles-23-32-2026-06-15.md`. **Still blocked:** same-universe origin school burden; EU27/NH-white lifetime rollup; descendant attribution (Akers-Boustan mined only); age-at-arrival/current-age NPV for Mexico stock. |
