...
pins and Census API notes in the [lane README](../infra/immigration-fiscal/mr_leads_papers_2026_09_21/README.md).

| Dataset / reference period | Storage and provenance | Variables and permitted use | Main limitation |
|---|---|---|---|
| ACS 2024 one-year PUMS, national tabulations | Tracked `mr_leads_papers_2026_09_21/derived/acs_care_inputs.csv`, regenerated by `pull_acs_care.py` (needs `CENSUS_API_KEY`); raw responses in ignored `_cache/` | Employed aides and nursing assistants (occupation codes 3601, 3602, 3603, 3605) by nativity, Mexican birthplace and Mexican origin; working-age population with less than one year of college by nativity and Mexican birthplace; population 65+ and 80+ by housing type and nativity | Weighted counts only, no standard errors pulled; institutional group quarters are not split by type; occupation is current or most recent job |
| CMS National Health Expenditure tables, 1970–2024 | `mr_leads_papers_2026_09_21/_cache/nhe-tables.zip` (ignored), 520,391 bytes, SHA256 `a09ef6d3e84e25d745047a47b6b08a0d96b303085b4c725b67ce67a0eb0c4420`, [CMS](https://www.cms.gov/files/zip/nhe-tables.zip) | Table 15 nursing care facilities by payer (2024: total $219.9bn, Medicaid $78.9bn, Medicare $47.3bn); Table 14 home health (total $169.4bn, Medicaid $38.2bn) | National totals for all ages; most Medicaid home-and-community waiver spending sits in Table 13, not Table 14 |
| Seven working papers, primary PDFs and text | `mr_leads_papers_2026_09_21/_cache/<slug>.pdf` and `.txt` (ignored); tracked reading notes in `notes/` | Estimates with page and table references; see each note's files-covered section | Working-paper versions; two published versions were not opened; image-only figures supplied no numbers |

## September 20–21 national and population series for the historical back-cast

Inputs for the [2005–2024 back-cast](immigration-historical-backcast-2026-09-20.md);
commands, pins and limits in the [lane README](../infra/immigration-fiscal/historical_backcast_2026_09_20/README.md).
None of these observes the group's own taxes or programme use before income-year 2024.

| Dataset / reference period | Storage and provenance | Variables and permitted use | Main limitation |
|---|---|---|---|
| ACS 1-year `B03001` and Selected Population Profile `S0201`, 2005–2024 | Tracked `historical_backcast_2026_09_20/inputs/acs_mexican_origin.csv`, regenerated by `pull_acs.py` (needs `CENSUS_API_KEY`) | National Mexican-origin count every year; per-capita income, median age, median earnings of full-time year-round workers by sex and median household income for the Mexican group and the total, 2008 onward | No 2020 release; profile endpoint starts 2008 and failed for 2010. Group code is `401` to 2022 and `4015` from 2023, variable codes move yearly and `S0201PR_*` Puerto Rico variants share labels: resolve by label on `S0201_\d+E` only. Self-identified origin, not the account's birthplace-plus-identification union (ratio 1.049 in 2024) |
| BEA NIPA Section 7, Table 7.1 midperiod population, 1929–2025 | `historical_backcast_2026_09_20/_cache/Section7All_xls.xlsx` (ignored), 1,003,641 bytes, SHA256 `ce107c8ce92393613c0abae38156afcfaef8ab571eedf0302dc09ca44738b9ef`, [BEA](https://apps.bea.gov/national/Release/XLS/Survey/Section7All_xls.xlsx), published 2026-08-26 | Resident denominator by year for per-capita national series | 340.095m in 2024 against the account's 340.111m; the back-cast levels it to the account |
| BEA NIPA Sections 1 and 3, annual history (already held) | `sources/immigration-fiscal/data/external/bea_nipa/`; hashes enforced by `backcast.py` | Table 3.1 receipt lines, 3.12 benefits by programme, 3.13 subsidies, 3.17 consumption by function, 1.1.9 deflator, all 2005–2025 | National totals only; no origin dimension |
...
- Quirks/use: new legal permanent residents of 2003–04 only, no unauthorized population; sampling weights documented in `38031-Documentation-sampling_weights.pdf`; ICPSR missing-value codes in the TSVs. Intended for religion by earnings among new green-card holders (HUMAN.md ask of September 21). Executed 2026-09-22 in [`nis2003_religion_earnings_2026_09_22`](../infra/immigration-fiscal/nis2003_religion_earnings_2026_09_22/RESULT.md) (ladder 179). Correction to the key-variable line above: employment and the pay rate are in Section C (DS0006, DS0007: `C1`, `C48_1PPP`, `C49_1PPP`) and twelve-month wage income in Section G (DS0011, DS0012: `G7APPP`, spouse path `G16PPP` in DS0054); Section H (DS0013, DS0014) holds assets and housing and served only a home-ownership anchor.

### PEW_US_MUSLIMS_2017 — 2017 Survey of U.S. Muslims, public-use

- Source/acquired: Pew Research Center dataset download (questionnaire, codebook, SPSS file, full report), downloaded by the operator with a Pew account, September 22, 2026.
- Local/size: `sources/immigration-fiscal/data/external/pew/Pew-2017-US-Muslims.zip`, 6,121,578 bytes; SPSS `2017USMuslimPublicData - checked.sav`, 1,001 respondents, 222 variables; `Codebook`, `Muslim-American-Final-Questionnaire.pdf`, `U.S.-MUSLIMS-FULL-REPORT.pdf`. SHA256 in the raw-file manifest.
- Key variables: `respondent_birthregion2`, `father_birthregion2`, `mother_birthregion2` (birth countries collapsed to regions), `citizen`, `income` (2016 family income bands), `educrec`, `fertREC` (children ever born), `party`/`partyln`, attitude items (`qa2`, `qb2*`, `qc11*`), `weight` (main weight; further weights per the codebook).
- Quirks/use: self-identified Muslim adults only, 2017 cross-section; birthplace is region, not country; income is banded. The only US religion-observed file with nativity and income that we could obtain quickly (HUMAN.md ask of September 21). Not yet used.

### IPUMS_CPS_ASEC_1994_2025_2NDGEN — parental birthplace and outcomes, 32 ASEC files

- Source/acquired: IPUMS CPS extract 1 on the operator's account, submitted and downloaded through the IPUMS API by `infra/immigration-fiscal/acquire/ipums_cps_second_gen.py` on September 22, 2026 (extract ready in 144 s; the first transfer broke at 26 MB and was resumed).
