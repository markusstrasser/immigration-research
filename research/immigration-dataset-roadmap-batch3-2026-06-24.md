...
**Limit / honesty:** ITIN holder ≠ unauthorized (also includes some nonresidents, dependents, certain visa holders). ITIN filers omit unauthorized workers using other identifiers or not filing. The report gives aggregates; an ITIN/status-identifying microdata extract in the public-use file has not been verified. [SOURCE: taxpayeradvocate.irs.gov ARC24 Research Study 3] [GAP]
**Graph fit:** National federal income-tax component, with no clean state/origin join. It cannot directly validate an all-tax undocumented total such as ITEP's $96.7B because the populations and tax bases differ. [INFERENCE]
...
**Limit / honesty:** ESF mismatches include other workers and miss unauthorized earnings that match an identifier, so ESF is neither a demonstrated upper nor lower bound on all unauthorized earnings. W-2s are not unique people, and cumulative wages are neither annual payroll taxes nor net lifetime contributions. Suspended earnings can later be credited, and immigration status can change; these reports do not prove that every contributing worker never receives benefits. NWALIEN identifies a nonwork-SSN administrative group, not all unauthorized people or their current work authorization. Aggregate reports only; microdata restricted. [SOURCE: oig-files.ssa.gov A-03-18-50537] [INFERENCE]
...
**Key variables:** **immigrant status of inventor** (the field PatentsView lacks), patent counts, citation-weighted and market-value-weighted output, collaboration networks, technology field, county. The February 2025 manuscript reports 16% of inventors, 23% of patents, and 32% of modeled aggregate innovation including spillovers. The earlier 36% is a different working-paper version, not another patent share. Authorship and spillover attribution are different quantities. [SOURCE: https://web.stanford.edu/~diamondr/BDMP.pdf] [INFERENCE]
...
- **2026-09-05:** Corrected ITIN/legal-status and ESF stock/flow/bound interpretations and the inventor authorship-versus-attribution figures under the [material inference repair](../decisions/2026-09-05-material-inference-repair.md). These are candidate-source descriptions, not claims that restricted microdata or unverified status variables are available. See the [recent acquisition register](immigration-dataset-proxy-refresh-2026-09-05.md) for files actually procured.
