...
research/immigration-INDEX.md                      |  8 ++-
 ...
 .../analyze_internal_vs_immigrant_newcomers.py     | 68 +++++++++++-----------
...
..                                       .. 81E (`Moved from abroad` in the past
..                                       ..  year) so the
...
sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 1/5 --- Python
  ...
  7 2. Compute county-level recent-arri    7 2. Compute county-level moved-from-
  . val foreign-born (ACS B05005)          . abroad in the past year (ACS B07001
  .                                        . _081E)
...
sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 2/5 --- Python
 ...
 .. ation-US (excludes foreign)           .. ation-US (excludes foreign)
...
sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 3/5 --- Python
 ...
 .. ry; sum 2010-later."""                .. E."""
 75     url = "https://api.census.gov/d   75     url = "https://api.census.gov/d
 .. ata/2022/acs/acs5"                    .. ata/2022/acs/acs5"
 76     # B05005_002E = entered 2010 or   .. 
 ..  later (foreign-born subtotal)        .. 
 77     params = {                        76     params = {
 78         "get": "B05005_002E",         77         "get": "B07001_081E",
 79         "for": "county:*",            78         "for": "county:*",
 80     }                                 79     }
 81     r = requests.get(url, params=pa   80     r = requests.get(url, params=pa
 .. rams, timeout=120)                    .. rams, timeout=120)
 82     r.raise_for_status()              81     r.raise_for_status()
 83     data = r.json()                   82     data = r.json()
 84     df = pd.DataFrame(data[1:], col   83     df = pd.DataFrame(data[1:], col
 .. umns=data[0])                         .. umns=data[0])
 85     df["recent_fb_2010plus"] = pd.t   84     df["moved_from_abroad_past_year
 .. o_numeric(df["B05005_002E"], errors   .. "] = pd.to_numeric(df["B07001_081E"
 .. ="coerce")                            .. ], errors="coerce")
 86     df["state_fips"] = df["state"].   85     df["state_fips"] = df["state"].
 .. astype(str).str.zfill(2)              .. astype(str).str.zfill(2)
 87     df["county_fips"] = df["county"   86     df["county_fips"] = df["county"
 .. ].astype(str).str.zfill(3)            .. ].astype(str).str.zfill(3)
 88     df["fips5"] = df["state_fips"]    87     df["fips5"] = df["state_fips"] 
 .. + df["county_fips"]                   .. + df["county_fips"]
 89     return df[["fips5", "recent_fb_   88     return df[["fips5", "moved_from
 .. 2010plus"]]                           .. _abroad_past_year"]]
 90                                       89 
 91                                       90 
 92 def main():                           91 def main():

sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 4/5 --- Python
 98     acs = fetch_acs_county_populati   97     acs = fetch_acs_county_populati
 .. on_and_recent_fb()                    .. on_and_recent_fb()
 99     print(f"  {len(acs)} ACS counti   98     print(f"  {len(acs)} ACS counti
 .. es")                                  .. es")
100                                       99 
101     print("Loading ACS recent-arriv  100     print("Loading ACS moved-from-a
... al FB (B05005)...")                  ... broad past year (B07001_081E)...")
102     try:                             101     try:
103         rfb = fetch_acs_recent_fb()  102         rfb = fetch_acs_moved_from_
...                                      ... abroad()
104         print(f"  {len(rfb)} county  103         print(f"  {len(rfb)} county
...  recent-FB rows")                    ...  moved-from-abroad rows")
105         acs = acs.merge(rfb, on="fi  104         acs = acs.merge(rfb, on="fi
... ps5", how="left")                    ... ps5", how="left")
106     except Exception as e:           105     except Exception as e:
107         print(f"  recent-FB fetch f  106         print(f"  moved-from-abroad
... ailed: {e}")                         ...  fetch failed: {e}")
108                                      107 
109     merged = acs.merge(soi, left_on  108     merged = acs.merge(soi, left_on
... ="fips5", right_on="dest_fips5", ho  ... ="fips5", right_on="dest_fips5", ho
... w="left")                            ... w="left")
110     merged["us_inflow_persons"] = m  109     merged["us_inflow_persons"] = m
... erged["us_inflow_persons"].fillna(0  ... erged["us_inflow_persons"].fillna(0
... )                                    ... )
111     merged["us_inflow_share"] = mer  110     merged["us_inflow_share"] = mer
... ged["us_inflow_persons"] / merged["  ... ged["us_inflow_persons"] / merged["
... totpop"]                             ... totpop"]
112     if "recent_fb_2010plus" in merg  111     if "moved_from_abroad_past_year
...
... / 12                                 ... 
115         merged["recent_fb_annual_sh  112         merged["moved_from_abroad_s
... are"] = merged["recent_fb_annual_av  ... hare"] = merged["moved_from_abroad_
... g"] / merged["totpop"]               ... past_year"] / merged["totpop"]
116     merged.to_parquet(OUT / "county  113     merged.to_parquet(OUT / "county
... _newcomer_comparison.parquet", inde  ... _newcomer_comparison.parquet", inde
... x=False)                             ... x=False)
117     print(f"\nWrote {OUT/'county_ne  114     print(f"\nWrote {OUT/'county_ne
... wcomer_comparison.parquet'}: {len(m  ... wcomer_comparison.parquet'}: {len(m
... erged)} counties")                   ... erged)} counties")
118                                      115 
119     # Stylized facts                 116     # Stylized facts
120     print("\n=== Stylized fact 1: n  117     print("\n=== Stylized fact 1: d
... ative-newcomer inflow vs immigrant-  ... omestic-newcomer inflow vs immigran
... newcomer inflow ===")                ... t-newcomer inflow ===")
121     sub = merged.dropna(subset=["us  118     sub = merged.dropna(subset=["us
... _inflow_persons", "totpop"])         ... _inflow_persons", "totpop"])
122     sub = sub[sub["totpop"] >= 1000  119     sub = sub[sub["totpop"] >= 1000
... 0]  # filter tiny counties           ... 0]  # filter tiny counties
123     print(f"Counties (≥10k pop): {l  120     print(f"Counties (≥10k pop): {l
... en(sub)}")                           ... en(sub)}")
124     print(f"Median US-inflow share   121     print(f"Median US-inflow share 
... / yr:    {sub['us_inflow_share'].me  ... / yr:    {sub['us_inflow_share'].me
... dian():.4f} ({sub['us_inflow_share'  ... dian():.4f} ({sub['us_inflow_share'
... ].median()*100:.2f}%)")              ... ].median()*100:.2f}%)")
125     if "recent_fb_annual_share" in   122     if "moved_from_abroad_share" in
... sub.columns:                         ...  sub.columns:
126         print(f"Median recent-FB an  123         print(
... nual share: {sub['recent_fb_annual_  ... 
... share'].median():.4f} ({sub['recent  ... 
... _fb_annual_share'].median()*100:.2f  ... 
... }%)")                                ... 
...                                      124             f"Median moved-from-abr
...                                      ... oad share: {sub['moved_from_abroad_
...                                      ... share'].median():.4f} "
...                                      125             f"({sub['moved_from_abr
...                                      ... oad_share'].median()*100:.2f}%)"
...                                      126         )
127         # Ratio                      127         # Ratio
128         ratio = sub["us_inflow_shar  128         ratio = sub["us_inflow_shar
... e"] / sub["recent_fb_annual_share"]  ... e"] / sub["moved_from_abroad_share"
...
... lse))                                ... lse))
134                                      134 
135     if "recent_fb_annual_share" in   135     if "moved_from_abroad_share" in
... sub.columns:                         ...  sub.columns:
136         print("\n=== Stylized fact   136         print("\n=== Stylized fact 
... 3: counties with HIGHEST recent-FB   ... 3: counties with HIGHEST moved-from
... inflow share ===")                   ... -abroad share ===")
137         top_fb = sub.nlargest(15, "  137         top_fb = sub.nlargest(15, "
...
... t"]]                                 ... edian_rent"]]
138         print(top_fb.to_string(inde  138         print(top_fb.to_string(inde
... x=False))                            ... x=False))
139                                      139 
140     # Quintile comparison            140     # Quintile comparison
141     print("\n=== Stylized fact 4: r  141     print("\n=== Stylized fact 4: r
... ent and income by inflow type quint  ... ent and income by inflow type quint
... ile ===")                            ... ile ===")
142     sub["us_q"] = pd.qcut(sub["us_i  142     sub["us_q"] = pd.qcut(sub["us_i
... nflow_share"], 5, labels=["Q1 lowes  ... nflow_share"], 5, labels=["Q1 lowes
... t", "Q2", "Q3", "Q4", "Q5 highest"]  ... t", "Q2", "Q3", "Q4", "Q5 highest"]
... )                                    ... )
143     print("\nBy US-NATIVE inflow qu  143     print("\nBy US-domestic inflow 
... intile:")                            ... quintile:")
144     print(sub.groupby("us_q", obser  144     print(sub.groupby("us_q", obser
... ved=True).agg(                       ... ved=True).agg(
145         n=("NAME", "count"),         145         n=("NAME", "count"),
146         median_inflow_share=("us_in  146         median_inflow_share=("us_in
...
149         median_fb_share_overall=("f  149         median_fb_share_overall=("f
... b_share", "median"),                 ... b_share", "median"),
150     ).to_string())                   150     ).to_string())
151                                      151 
152     if "recent_fb_annual_share" in   152     if "moved_from_abroad_share" in
... sub.columns:                         ...  sub.columns:
153         sub2 = sub.dropna(subset=["  153         sub2 = sub.dropna(subset=["
... recent_fb_annual_share"]).copy()     ... moved_from_abroad_share"]).copy()
154         sub2["fb_q"] = pd.qcut(sub2  154         sub2["fb_q"] = pd.qcut(sub2
...
155         print("\nBy RECENT-FOREIGN-  155         print("\nBy MOVED-FROM-ABRO
... BORN inflow quintile:")              ... AD inflow quintile:")
156         print(sub2.groupby("fb_q",   156         print(sub2.groupby("fb_q", 
... observed=True).agg(                  ... observed=True).agg(
157             n=("NAME", "count"),     157             n=("NAME", "count"),
158             median_inflow_share=("r  158             median_inflow_share=("m
... ecent_fb_annual_share", "median"),   ... oved_from_abroad_share", "median"),
159             median_rent=("median_re  159             median_rent=("median_re
... nt", "median"),                      ... nt", "median"),
160             median_hh_income=("medi  160             median_hh_income=("medi
... an_hh_income", "median"),            ... an_hh_income", "median"),
161             median_us_inflow_share=  161             median_us_inflow_share=
...
... [truncated for review packet] ...
 ...
 Migration-US aggregate, not a native-incumbent-only measure.",
        },
...
2. Compute county-level moved-from-abroad in the past year (ACS B07001_081E)
...
def fetch_acs_moved_from_abroad() -> pd.DataFrame:
    ...
    df["county_fips"] = df["county"].astype(str).str.zfill(3)
    df["fips5"] = df["state_fips"] + df["county_fips"]
    return df[["fips5", "moved_from_abroad_past_year"]]
...
def main():
    ...
    if "moved_from_abroad_share" in sub.columns:
        ...
        # Ratio
        ratio = sub["us_inflow_share"] / sub["moved_from_abroad_share"].replace(0, np.nan)
    ...
    print("\nBy US-domestic inflow quintile:")
...
6. **(Cross-cut) Internal vs immigrant newcomers.** IRS SOI county inflow
   (`Total Migration-US`, not native-only filers) joined to ACS B07001_081E (`Moved from abroad` in the past year) so the
...
5. The right updated claim is:
   ...
   - county employment effects remain harder to separate from pre
...
```text
{
  "sample_accounting": {
    ...
    "clean_window_counts": {
      "wage_2017_2018_nonmissing": 2390,
      "employment_2017_2018_nonmissing": 2390,
...
```text
window_type,outcome_window,base_year,end_year,label,touches_2020,window_role,contaminated,reason,metric,missing_input_count,nonmissing_count,zero_base_count,nondisclosure_missing_base_count,nondisclosure_missing_end_count,window_key,beta,t_stat,p_value,adj_r2,n
lead_exposure_clean_presurge_window,wage_pre_2017_2018,2017,2018,clean_pre_covid_early,False,presurge_check,False,Second clean annual pre-COVID window for pretrend checks.,weekly_wage,0,2390,0,0,0,qcew_wkly_wage_log_change_2017_2018,0.0016229957423122185,2.4647033735124912,0.013712671951059598,0.054605856774141426,2313
lead_exposure_clean_presurge_window,wage_pre_2018_2019,2018,2019,clean_pre_covid,False,presurge_check,False,Clean annual pre-COVID window.,weekly_wage,0,2390,0,0,0,qcew_wkly_wage_log_change_2018_2019,-0.000783966076535148,-1.453059607653236,0.14620720468802909,0.035086780804650775,2313
lead_exposure_clean_presurge_window,employment_pre_2017_2018,2017,2018,clean_pre_covid_early,False,presurge_check,False,Second clean annual pre-COVID window for pretrend checks.,employment,0,2390,0,0,0,qcew_employment_log_change_2017_2018,-0.0024603136084290498,-3.330555906554242,0.0008667275695865356,0.054464138093704095,2313
lead_exposure_clean_presurge_window,employment_pre_2018_2019,2018,2019,clean_pre_covid,False,presurge_check,False,Clean annual pre-COVID window.,employment,0,2390,0,0,0,qcew_employment_log_change_2018_2019,-0.002993587621363311,-4.804650688200625,1.5502191144972167e-06,0.0920767070080648,2313
post_with_presurge_wage_controls,wage_post_2023_2024,2023,2024,post_surge_clean,False,post_period,False,Clean post window that does not touch 2020.,weekly_wage,0,2390,0,0,0,qcew_wkly_wage_log_change_2023_2024,-0.0012625684855220904,-2.1223694181607393,0.033806728223312904,0.02276361779284597,2313
post_with_presurge_employment_controls,employment_post_2023_2024,2023,2024,post_surge_clean,False,post_period,False,Clean post window that does not touch 2020.,employment,0,2390,0,0,0,qcew_employment_log_change_2023_2024,-0.0023037439324560685,-3.2323705036357797,0.0012276776530951072,0.036136509454634425,2313
```
