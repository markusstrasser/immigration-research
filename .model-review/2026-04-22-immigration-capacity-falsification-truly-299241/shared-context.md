...
def build_qcew_panel() -> pd.DataFrame:
    ...
    dups = qcew[qcew.duplicated(["fips5", "year"], keep=False)].copy()
    if len(dups):
        sample = dups[["fips5", "year"]].drop_duplicates().head(10).to_dict(orient="records")
        raise ValueError(f"Unexpected duplicate QCEW county-years: {sample}")
    qcew = qcew.sort_values(["fips5", "year"]).reset_index(drop=True)
    return qcew
...
rquet(QCEW_OUT, index=False)
    ...
    panel["irs_inflow_row_missing"] = panel["us_inflow_persons_2022_23"].isna()
    panel["irs_outflow_row_missing"] = panel["us_outflow_persons_2022_23"].isna()
    panel["qcew_row_missing"] = panel["annual_avg_emplvl_2021"].isna()
    for col in (
...
2. [county_capacity_falsification_summary.json](/Users/alien/Projects/research/sources/immigration-causal/data

... [truncated for review packet] ...

**not** justify a sweeping “real family across outcomes” line. [INFERENCE]
...
3. wage-side post effects remain after annual presurge/onset controls [SOURCE]
4. a wage-tuned exploratory threshold search beats a null search on performance [SOURCE]
5. the location of that exploratory threshold remains diffuse [SOURCE]
...
Too strong. Even after moving the permit baseline back to `2017–2019`, one clean annual pre-COVID window is not enough to call the split settled. [SOURCE]
...
ustness needed to be stronger than leave-one-state-out [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-83e8f8/verified-disposition.md]
...
## Phase 8: corrected falsification narrows the story, but does not cleanly settle the wage-employment split
