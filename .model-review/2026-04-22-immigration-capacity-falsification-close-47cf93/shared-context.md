...
Focus on:
- whether the county panel rebuild in `build_county_outcome_panel.py` correctly materializes the clean `2018–2019` pre-COVID window, marks `2020`-touching windows as contaminated/descriptive only, and handles QCEW nondisclosure rows defensibly
- permutation / placebo / holdout-threshold logic in `analyze_capacity_falsification.py`
- whether the new memos overstate causal confidence after the falsification pass, especially around threshold stability and the quarantined IRS domestic-migration channel
...
- `sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_validation.csv`
- `sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_null.csv`
...
```text
research/immigration-INDEX.md                                |  6 ++++--
 sources/immigration-causal/README.md                         |  2 +-
 .../scripts/analyze_internal_vs_immigrant_newcomers.py       | 12 ++++++------
...
75 | `immigration-causal-everify-card-v  75 | `immigration-causal-everify-card-v
...
76 | `immigration-causal-saiz-elasticit  76 | `immigration-causal-saiz-elasticit
...
77 | `immigration-causal-paradigm-escap  77 | `immigration-causal-paradigm-escap
...
78 | `immigration-causal-internal-vs-im  78 | `immigration-causal-internal-vs-im
.. migrant-newcomers.md` | IRS SOI × AC  .. migrant-newcomers.md` | IRS SOI × AC
.. S — native county inflow is 33× immi  .. S — domestic county inflow is 33× im
.. grant inflow | Treating "newcomer bu  .. migrant inflow | Treating "newcomer 
.. rden" as immigration-driven by defau  .. burden" as immigration-driven by def
.. lt |                                  .. ault |
79 | `immigration-causal-surge-2021-202  79 | `immigration-causal-surge-2021-202
...
80 | `immigration-county-outcome-panel-  80 | `immigration-county-outcome-panel-
...
..                                       .. ve-incumbent-only |
81 | `immigration-capacity-frontier-202  81 | `immigration-capacity-frontier-202
...
..                                       82 | `immigration-capacity-falsificatio
..                                       .. n-2026-04-21.md` | Corrected falsifi
..                                       .. cation pass with the clean `2018–201
..                                       .. 9` annual pre-COVID check, `1,000`-d
..                                       .. raw permutation inference, division/
..                                       .. state leave-out, explicit window met
..                                       .. adata, and wage-threshold null bench
..                                       .. marking | Asking which parts of the 
..                                       .. new flow-capacity story still surviv
..                                       .. e after fixing the earlier placebo b
..                                       .. ug |
..                                       83 | `immigration-reasoning-evolution-2
..                                       .. 026-04-21.md` | Narrative provenance
..                                       ..  trace of how the repo’s immigration
..                                       ..  reasoning changed, including the `/
..                                       .. critique close` correction that spli
..                                       .. t the wage and employment stories | 
..                                       .. Wanting the evolution of reasoning i
..                                       .. tself traced rather than only the la
..                                       .. test stance |
82                                       84 
83 ## Raw Data & Warehouse               85 ## Raw Data & Warehouse
84                                       86 
...
.. B05005 (recent foreign-born) so the   .. -only filers) joined to ACS B05005 (
..                                       .. recent foreign-born) so the
44    reader can ask "is the policy-rel  44    reader can ask "is the policy-rel
.. evant unit the immigrant or the newc  .. evant unit the immigrant or the newc
.. omer                                  .. omer
45    in general?" Script: `analyze_int  45    in general?" Script: `analyze_int
.. ernal_vs_immigrant_newcomers.py`.     .. ernal_vs_immigrant_newcomers.py`.
46                                       46 

sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 1/4 --- Python
  1 """Internal-native migration vs int    1 """Domestic internal migration vs i
  . ernational immigration as comparabl    . nternational immigration as compara
  . e shocks.                              . ble shocks.
  2                                        2 
  3 Reframes: is the 'immigrant' or the    3 Reframes: is the 'immigrant' or the
  .  'newcomer in general' the right po    .  'newcomer in general' the right po
  . licy unit?                             . licy unit?
  4                                        4 
  5 Approach:                              5 Approach:
  6 1. Compute county-level inflow of n    6 1. Compute county-level domestic US
  . ative US filers (IRS SOI 2022-23, c    .  inflow aggregate (IRS SOI 2022-23,
  . ode y1=97)                             .  code y1=97)
  7 2. Compute county-level recent-arri    7 2. Compute county-level recent-arri
  . val foreign-born (ACS B05005)          . val foreign-born (ACS B05005)
  8 3. For each county: compute "newcom    8 3. For each county: compute "newcom
  . er share" by type                      . er share" by type
  9 4. Test: do counties receiving high    9 4. Test: do counties receiving high
  .  native-migrant inflows show same s    .  domestic-migrant inflows show same
  . chool/housing/wage                     .  school/housing/wage
 10    patterns as those receiving high   10    patterns as those receiving high
 ..  immigrant inflows?                   ..  immigrant inflows?
 11                                       11 
...
sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 2/4 --- Python
 23                                       23 
 24                                       24 
...
sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 3/4 --- Python
117     print(f"\nWrote {OUT/'county_ne  117     print(f"\nWrote {OUT/'county_ne
... wcomer_comparison.parquet'}: {len(m  ... wcomer_comparison.parquet'}: {len(m
... erged)} counties")                   ... erged)} counties")
118                                      118 
119     # Stylized facts                 119     # Stylized facts
120     print("\n=== Stylized fact 1: n  120     print("\n=== Stylized fact 1: d
... ative-newcomer inflow vs immigrant-  ... omestic-newcomer inflow vs immigran
... newcomer inflow ===")                ... t-newcomer inflow ===")
121     sub = merged.dropna(subset=["us  121     sub = merged.dropna(subset=["us
... _inflow_persons", "totpop"])         ... _inflow_persons", "totpop"])
122     sub = sub[sub["totpop"] >= 1000  122     sub = sub[sub["totpop"] >= 1000
... 0]  # filter tiny counties           ... 0]  # filter tiny counties
123     print(f"Counties (≥10k pop): {l  123     print(f"Counties (≥10k pop): {l
... en(sub)}")                           ... en(sub)}")

sources/immigration-causal/scripts/analyze_internal_vs_immigrant_newcomers.py --- 4/4 --- Python
128         ratio = sub["us_inflow_shar  128         ratio = sub["us_inflow_shar
... e"] / sub["recent_fb_annual_share"]  ... e"] / sub["recent_fb_annual_share"]
... .replace(0, np.nan)                  ... .replace(0, np.nan)
129         print(f"Median US-inflow /   129         print(f"Median US-inflow / 
... FB-inflow ratio: {ratio.median():.1  ... FB-inflow ratio: {ratio.median():.1
... f}x")                                ... f}x")
130                                      130 
131     print("\n=== Stylized fact 2: c  131     print("\n=== Stylized fact 2: c
... ounties with HIGHEST native inflow   ... ounties with HIGHEST domestic inflo
... ===")                                ... w ===")
132     top_us = sub.nlargest(15, "us_i  132     top_us = sub.nlargest(15, "us_i
... nflow_share")[["NAME", "totpop", "u  ... nflow_share")[["NAME", "totpop", "u
... s_inflow_persons", "us_inflow_share  ... s_inflow_persons", "us_inflow_share
... ", "median_rent"]]                   ... ", "median_rent"]]
133     print(top_us.to_string(index=Fa  133     print(top_us.to_string(index=Fa
... lse))                                ... lse))
134                                      134
```
...
... [truncated for review packet] ...
   ...
   "rows": int(len(panel)),
        "elect_counties": int(elect["fips5"].nunique()),
        "qcew_counties": int(qcew["fips5"].nunique()),
        "qcew_missing_counties": int(panel["qcew_row_missing"].sum()),
        "irs_inflow_missing_rows": int(panel["irs_inflow_row_missing"].sum()),
        "irs_outflow_missing_rows": int(panel["irs_outflow_row_missing"].sum()),
        "qcew_non_disclosure_rows_by_year": {},
        "irs_migration_layer": {
            "source": "IRS county-to-county migration files",
            "codes_used": ["97/000 inflow", "97/000 outflow"],
    ...
    for year in range(2018, 2025):
        sub = qcew[qcew["year"] == year]
        audit["qcew_non_disclosure_rows_by_year"][str(year)] = int((sub["disclosure_code"] == "N").sum())
    for year in range(2018, 2025):
        audit["qcew_missing_cells_by_year"][str(year)] = {
...
summary_rows = []
    ...
    if len(thresh_null):
        ...
        null_summary = {
            ...
            "delta_vs_actual_wage_sign_match": float(actual_wage["test_sign_matches_train"].mean() - thresh_null["test_sign_matches_train"].mean()),
            "delta_vs_actual_wage_median_abs_t": float(actual_wage["test_t"].abs().median() - thresh_null["test_abs_t"].median()),
            "actual_modal_share": float(surface_actual["selection_share"].max()),
            "null_modal_share": float(surface_null["selection_share"].max()),
...
```text
"""Domestic internal migration vs international immigration as comparable shocks.

Reframes: is the 'immigrant' or the 'newcomer in general' the right policy unit?

Approach:
1. Compute county-level domestic US inflow aggregate (IRS SOI 2022-23, code y1=97)
2. Compute county-level recent-arrival foreign-born (ACS B05005)
3. For each county: compute "newcomer share" by type
4. Test: do counties receiving high domestic-migrant inflows show same school/housing/wage
...
def main():
    ...
    # Stylized facts
    print("\n=== Stylized fact 1: domestic-newcomer inflow vs immigrant-newcomer inflow ===")
    ...
    print("\n=== Stylized fact 2: counties with HIGHEST domestic inflow ===")
...
6. **(Cross-cut) Internal vs immigrant newcomers.** IRS SOI county inflow
   (`Total Migration-US`, not native-only filers) joined to ACS B05005 (recent foreign-born) so the
...
2. The earlier “pretrend” critique was partly right for the **wrong test**. The old `2021–2022` placebo section was mislabeled because it used overlapping treatment windows. After rebuilding the panel and moving the permit baseline back to `2017–2019`, the clean annual pre-COVID window (`2018–2019`) still shows weak wage pretrend but negative employment pretrend. [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-83e8f8/verified-disposition.md] [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_pretrend_results.csv]
3. The annual-data causal split is now **provisional, not settled**:
   - `wage story`: still somewhat more promising, because the clean annual pre-COVID wage window (`2018–2019`) is weak and the post result survives a control for that window [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_pretrend_results.csv]
   - `employment story`: more suspicious, because even the clean `2018–2019` annual window is already negative [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_pretrend_results.csv]
   - but any stronger wage-versus-employment split that leans on `2019–2020` or `2020–2021` is contaminated by COVID-era annual averaging and should be treated as unverified pending quarterly or pre-2020-only reruns [INFERENCE]
4. The threshold result no longer supports a generic “real family across outcomes” claim. It now supports a **wage-tuned exploratory threshold search that beats the null on performance but not on stable location**. Holdout sign stability for wage beats the null strongly, while the selected cutoff surface is diffuse, transfer to `margin` is poor, and transfer to `employment` is only moderate. [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_validation.csv] [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_null.csv] [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_surface.csv]
5. The right updated claim is:
   ...
   - the current falsification claim set excludes the IRS domestic-migration layer becaus

... [truncated for review packet] ...

idation.csv] [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_null.csv]
...
This is too strong. [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_surface.csv] [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_validation.csv]

2. `The annual-data wage-versus-employment causal split is already settled.`  
Too strong. Even after moving the permit baseline back to `2017–2019`, one clean annual pre-COVID window is not enough to call the split settled. [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_pretrend_results.csv] [SOURCE: sources/immigration-causal/data/outcomes/county_outcome_panel_audit.json]

3. `The threshold story generalizes equally well to wages, employment, and politics.`  
False in the corrected pass. Margin transfer is poor. [SOURCE: sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_validation.csv]
...
2. `it likely operates as a real amplifier of local strain` [INFERENCE]
3. `annual data are more suggestive for wage than for employment, but the split is still provisional because the annual design has only one clean pre-COVID window` [INFERENCE]
...
2. `causal confidence on a clean wage-versus-employment split from the annual county panel`: low to moderate [INFERENCE]
...
The first draft of this memo treated `2021–2022` overlap windows as placebo/pretrend evidence. After `/critique close` flagged that as a real semantics bug, the county panel was rebuilt with earlier QCEW windows, the falsification script was rewritten around a true presurge permit baseline, and the threshold section was downgraded again once the exported threshold surface showed diffuse cutoff selection. A later close review then forced one more correction: the annual design should rely on the clean `2018–2019` pre-COVID window and a `2017–2019` permit baseline, not on windows touching `2020`, and the wage-versus-employment split remains provisional even after that cleanup. [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-83e8f8/verified-disposition.md] [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-final-36f586/verified-disposition.md]
```
...
tadata, and rewriting the falsification script around a true `2017–2019` permit baseline, the corrected picture is:
...
2. the clean annual pre-COVID wage window (`2018–2019`) is weak, so the old blanket wage pretrend critique was too broad [SOURCE: research/immigration-capacity-falsification-2026-04-21.md]
3. the clean annual pre-COVID employment window (`2018–2019`) is already negative, which keeps employment more suspicious [SOURCE: research/immigration-capacity-falsification-2026-04-21.md]
4. post-`2023–2024` wage and employment effects still survive annual controls, but one clean annual pre-COVID window is not enough to treat the wage-versus-employment split as settled [SOURCE: research/immigration-capacity-falsification-2026-04-21.md]
...
1. `wage-side surge amplification`: somewhat more plausible than before
2. `employment-side surge amplification`: much less clean
3. `wage-versus-employment causal split from the annual panel`: still provisional because the annual design ultimately rests on one clean pre-COVID window
4. `generic county threshold story across all outcomes`: too strong
...
The first draft of this file narrated the earlier falsification pass as if its “pretrend” downgrade were already sound. After the corrected county panel and first review artifacts landed, the file was rewritten to distinguish the bug-triggered correction from the evidence that still survives after that correction. A later review forced one more downgrade: the annual design should lean only on the clean `2018–2019` pre-COVID window and a `2017–2019` permit baseline, which still leaves the wage-versus-employment split provisional rather than settled. [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-83e8f8/verified-disposition.md] [SOURCE: .model-review/2026-04-22-immigration-capacity-falsification-final-36f586/verified-disposition.md] [SOURCE: research/immigration-capacity-falsification-2026-04-21.md]
```
...
1. **The new county labor result is a wage-growth story, not an employment-collapse story.** In counties with very high recent immigration and low permit throughput, countywide weekly wage growth from `2021` to `2024` is about `1.5 pp` lower, while employment growth is not significantly lower. [DATA]
2. **Backlash and labor do not load on exactly the same moderator.** In the top recent-immigration counties, both `low permit` and `high rent burden` predict more GOP shift, but the wage-growth penalty attaches to `low permit`, while IRS domestic net out-migration attaches more to `high rent burden`. [DATA]
...
... [truncated for review packet] ...
 ...
 `p≈0.0026`, while the rent interaction stays small and not significant [DATA]
...
1. median employment growth rises from about `2.72 pp` in the lowest-permit quartile to about `8.55 pp` in the highest-permit quartile [DATA]
2. median weekly wage growth rises from about `11.00 pp` to about `13.13 pp` [DATA]
3. median GOP margin shift falls from about `+5.12 pp` to about `+3.79 pp` [DATA]
...
1. `high_recent_fb × low_permit` is near zero on IRS domestic net migration share, `p≈0.69` [DATA]
2. `high_recent_fb × high_rent_burden ≈ -0.21 pp`, `t≈-2.85`, `p≈0.0044` [DATA]
...
1. `high_recent_fb × low_permit ≈ +0.75 pp`, `p≈0.064` [DATA]
2. `high_recent_fb × high_rent_burden ≈ +0.75 pp`, `p≈0.038` [DATA]
...
1. the first broad county labor response is slower wage growth, not strong job loss [DATA]
2. permit throughput is the wage-side moderator [DATA]
3. rent burden is the cleaner IRS domestic-migration moderator [DATA]
4. politics sits on top of both channels [DATA] [INFERENCE]

## Best current causal formulation

The best public-data formulation is now:

1. **`low permit throughput` is the main county wage-growth bottleneck under high recent immigration.** [DATA]
2. **`high rent burden` is the stronger IRS domestic-migration signal.** [DATA]
3. **`political backlash` responds to both scarcity and affordability.** [DATA]
4. **`employment collapse` is not the clean county-level story in this panel.** [DATA]
...
2. IRS SOI migration is filer-based, not a full resident microflow. [INFERENCE]
3. This is still reduced-form county evidence, not a structural model with endogenous housing supply, relocation, and sector composition. [INFERENCE]
4. The shelter/legal-regime story still needs the receiver-node panel and ideally `HMIS/LSA` for a fuller local-capacity account. [SOURCE: research/immigration-surge-threshold-dataset-frontier-2026-04-21.md]
```
...
"rent exposure is just price discovery" |
...
| `immigration-causal-internal-vs-immigrant-newcomers.md` | IRS SOI × ACS — domestic county inflow is 33× immigrant inflow | Treating "newcomer burden" as immigration-driven by default |
...
| `immigration-capacity-falsification-2026-04-21.md` | Corrected falsification pass with the clean `2018–2019` annual pre-COVID check, `1,000`-draw permutation inference, division/state leave-out, explicit window metadata, and wage-threshold null benchmarking | Asking which parts of the new flow-capacity story still survive after fixing the earlier placebo bug |
...
```text
{
  ...
  "qcew_window_metadata": {
    "qcew_employment_log_change_2018_2019": {
      "base_year": 2018,
      "end_year": 2019,
      "label": "clean_pre_covid",
      "touches_2020": false,
    ...
    "qcew_wkly_wage_log_change_2018_2019": {
      "base_year": 2018,
      "end_year": 2019,
      "label": "clean_pre_covid",
      "touches_2020": false,
    ...
    "qcew_employment_log_change_2018_2020": {
      "base_year": 2018,
      "end_year": 2020,
      "label": "covid_overlap_long",
      "touches_2020": true,
    ...
    "qcew_wkly_wage_log_change_2018_2020": {
      "base_year": 2018,
      "end_year": 2020,
      "label": "covid_overlap_long",
      "touches_2020": true,
    ...
    "qcew_employment_log_change_2019_2020": {
      "base_year": 2019,
      "end_year": 2020,
      "label": "covid_onset",
      "touches_2020": true,
...
{
  "sample_accounting": {
    "analysis_counties_ge10k": 2390,
    "zero_current_permit_counties": 64,
    "zero_prebase_permit_counties": 77,
    "nonmissing_current_load_counties": 2326,
    "nonmissing_prebase_load_counties": 2313,
    "recent_fb_proxy": "ACS 2022 5-year B05005 entered-2010-or-later stock divided by 12; proxy for recent annualized foreign-born flow, not a sharp 2021-2024 arrival measure.",
    "load_measure": "recent annualized foreign-born proxy per annual permit unit; no persons-per-unit housing conversion is used in this falsification pass.",
    "clean_window_counts": {
      "wage_2018_2019_nonmissing": 2390,
      "employment_2018_2019_nonmissing": 2390,
      "wage_2023_2024_nonmissing": 2390,
      "employment_2023_2024_nonmissing": 2390
    },
    "qcew_window_metadata": {
      "qcew_wkly_wage_log_change_2018_2019": {
        "base_year": 2018,
      ...
      "qcew_wkly_wage_log_change_2023_2024": {
        "base_year": 2023,
        "end_year": 2024,
        "label": "post_surge_clean",
        "touches_2020": false,
      ...
      "qcew_employment_log_change_2023_2024": {
        "base_year": 2023,
        "end_year": 2024,
        "label": "post_surge_clean",
        "touches_2020": false,
      ...
      "qcew_wkly_wage_log_change_2021_2022": {
        "base_year": 2021,
        "end_year": 2022,
        "label": "surge_overlap_short",
        "touches_2020": false,
      ...
      "qcew_employment_log_change_2021_2022": {
        "base_year": 2021,
        "end_year": 2022,
        "label": "surge_overlap_short",
        "touches_2020": false,
    ...
    "excluded_channels": {
      "irs_domestic_migration": "Excluded from the falsification claim set because the IRS 97/000 layer is total domestic migration, not a native-incumbent-only outcome."
    }
  },
  "permutation": [
    {
      "outcome": "margin_shift",
      "observed_beta": 0.005446432108433827,
      "observed_t": 6.963445541091321,
      "observed_p": 3.320495572499784e-12,
      "perm_mean_t": -0.10269896228543555,
      "perm_sd_t": 0.9787374686528186,
...
666666666666
    ...
    {
      "source": "actual",
      "selected_recent_quantile": 75,
      "selected_permit_quantile": 25,
      "selection_count": 1,
      "selection_share": 0.016666666666666666
    },
    {
      "source": "actual",
      "selected_recent_quantile": 75,
      "selected_permit_quantile": 50,
      "selection_count": 3,
      "selection_share": 0.05
    },
    {
      "source": "actual",
      "selected_recent_quantile": 80,
      "selected_permit_quantile": 25,
      "selection_count": 1,
      "selection_share": 0.016666666666666666
    },
    ...
    {
      "source": "actual",
      "selected_recent_quantile": 85,
      "selected_permit_quantile": 25,
      "selection_count": 4,
      "selection_share": 0.06666666666666667
    },
    {
      "source": "null_search",
      "selected_recent_quantile": 60,
      "selected_permit_quantile": 25,
      "selection_count": 7,
      "selection_share": 0.11666666666666667
    },
    {
      "source": "null_search",
      "selected_recent_quantile": 60,
      "selected_permit_quantile": 50,
      "selection_count": 10,
      "selection_share": 0.16666666666666666
    },
    {
      "source": "null_search",
      "selected_recent_quantile": 65,
      "selected_permit_quantile": 25,
      "selection_count": 3,
      "selection_share": 0.05
    },
    ...
    },
    {
      ...
      "selected_permit_quantile": 50,
      "selection_count": 8,
      "selection_share": 0.13333333333333333
    }
  ],
  "monotonicity": [
    {
      "series": "median_margin_shift_pp",
      "spearman_rho": 0.8303030303030302,
      "spearman_p": 0.0029402270232795095,
      "decile_linear_slope_pp": 0.1753306580024602
    },
    {
      "series": "median_wage_change_pp",
      "spearman_rho": -0.9272727272727272,
      "spearman_p": 0.00011203450639397587,
      "decile_linear_slope_pp": -0.17505899534318328
    },
    {
      "series": "median_employment_change_pp",
      "spearman_rho": -0.9636363636363635,
      "spearman_p": 7.32097480952992e-06,
      "decile_linear_slope_pp": -0.20086847976404848
    }
  ]
}
```
...
```text
window_type,outcome_window,base_year,end_year,label,touches_2020,window_role,contaminated,reason,metric,missing_input_count,nonmissing_count,zero_base_count,window_key,beta,t_stat,p_value,adj_r2,n
pretrend_or_transition,wage_pre_2018_2019,2018,2019,clean_pre_covid,False,presurge_check,False,Clean annual pre-COVID window.,weekly_wage,0,2390,0,qcew_wkly_wage_log_change_2018_2019,-0.000783966076535148,-1.453059607653236,0.14620720468802909,0.035086780804650775,2313
pretrend_or_transition,employment_pre_2018_2019,2018,2019,clean_pre_covid,False,presurge_check,False,Clean annual pre-COVID window.,employment,0,2390,0,qcew_employment_log_change_2018_2019,-0.002993587621363311,-4.804650688200625,1.5502191144972167e-06,0.0920767070080648,2313
post_with_presurge_wage_controls,wage_post_2023_2024,2023,2024,post_surge_clean,False,post_period,False,Clean post window that does not touch 2020.,weekly_wage,0,2390,0,qcew_wkly_wage_log_change_2023_2024,-0.0013385296799294612,-2.2542476866667553,0.02418059034363397,0.020607706643166912,2313
post_with_presurge_employment_controls,employment_post_2023_2024,2023,2024,post_surge_clean,False,post_period,False,Clean post window that does not touch 2020.,employment,0,2390,0,qcew_employment_log_change_2023_2024,-0.0023445976732105094,-3.325438403620538,0.0008827960401280974,0.03601422136866406,2313
```

### sources/immigration-causal/data/outcomes/analysis/county_capacity_threshold_validation.csv

```text
split,selected_recent_quantile,selected_permit_quantile,outcome,train_beta,train_t,train_p,test_beta,test_t,test_p,test_sign_matches_train,n_train,n_test
0,65,25,wage,-0.023173873031795855,-3.254481995112885,0.0011359936074681306,-0.008716299513140369,-0.9270006389206177,0.3539262025519444,1,1186,1204
0,65,25,margin,-0.023173873031795855,-3.254481995112885,0.0011359936074681306,0.007771269652153948,1.2448289514991098,0.21319463159477958,0,1186,1204
0,65,25,employment,-0.023173873031795855,-3.254481995112885,0.0011359936074681306,-0.014140954185404762,-1.6833880948261475,0.09229998641375861,1,1186,1204
1,60,25,wage,-0.013877821411966054,-2.177539759392724,0.02944031864402699,-0.0111410881781094,-1.429543456431581,0.15284809401006053,1,1227,1163
1,60,25,margin,-0.013877821411966054,-2.177539759392724,0.02944031864402699,0.00836618595463331,1.430510552455571,0.15257053935671092,0,1227,1163
1,60,25,employment,-0.013877821411966054,-2.177539759392724,0.02944031864402699,-0.021643454789495836,-2.6966676948505843,0.007003712320502883,1,1227,1163
2,60,50,wage,-0.011263265913818647,-2.190212630246565,0.028508819671057135,-0.011336663703853141,-2.350868325510191,0.018729660480969933,1,1158,1232
2,60,50,margin,-0.011263265913818647,-2.190212630246565,0.028508819671057135,0.002779024196361486,0.7419292526017977,0.4581302011499213,0,1158,1232
2,60,50,employment,-0.011263265913818647,-2.190212630246565,0.028508819671057135,-0.002221562022115812,-0.3455400996410077,0.7296883693445759,1,1158,1232
3,85,25,wage,-0.021545414440017195,-2.6288226706945066,0.008568102702668837,-0.032648378471851064,-1.6469844827134301,0.09956123535753916,1,1167,1223
